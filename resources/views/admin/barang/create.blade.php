...
<div class="rightside">
  @include('layouts.navbaradmin')
    ...
    <div class="table-container flex">
        ...
        <form method="POST" class="flex-row" action="{{ route ('admin.barang.store') }}">
        @csrf
        <!-- Nama Barang -->
            ...
            <label class="text-xl flex-1" for="spesifikasi">Spesifikasi :</label>
             <select class=" text-lg font-medium p-3" style="flex: 4 1 0;" name="spesifikasi" id="spesifikasi">
                 ...
                 @foreach ($spesifikasi as $item)
                     <option value="{{ $item->spesifikasi }}">{{ $item->spesifikasi }}</option>
                 @endforeach
        ...
        <!-- Qty -->
         ...
         <label style="flex: 7 1 0;" class="text-xl" for="qty">Qty :</label>
             <input class="text-lg font-medium p-3" style="flex: 2 1 0;" type="number" name="qty" placeholder="Qty">
             <div style="flex: 20 1 0;"></div>
         </div>

        <div class="flex flex-col">
            <button class="text-lg w-40 border border-solid rounded-2xl border-stone-950 h-10 place-self-end" style="background-color:#7895cb"" type="submit">Simpan</button>
        </div>
